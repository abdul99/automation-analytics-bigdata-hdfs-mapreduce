package com.abdul.hadoop.mapreduce.pattern.delimiterend;
...
public class AtomCount {   
    ...
    public static class TokenizerMapper extends Mapper<Object, Text, Text, IntWritable> {
        ...
        public void map(Object key, Text value, Context context
        	       ...
        	       System.out.println("value passed : " + value.toString());
                    StringTokenizer itr = new StringTokenizer(value.toString());
                    while (itr.hasMoreTokens()) {
                    	haCount.set(itr.nextToken());
    ...
    public static void main(String[] args) throws Exception {
       ...
       /* if (otherArgs.length != 2) {
        ...
        if (args.length == 0) {
            args = new String[2];
            args[0] = "src/main/resources/cdkin";
            args[1] = "src/main/resources/cdkout";
            FileUtils.deleteDirectory(new File(args[1]));
        }
        
        Job job = Job.getInstance(conf, "customformat");
     //   Job job = new Job(conf, "haCount count");
        job.setJarByClass(AtomCount.class);
